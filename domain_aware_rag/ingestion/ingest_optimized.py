...
class EmbeddingStrategy(Enum):
    EMBED_ALL = "embed_all"
    EMBED_ENTITY_RICH = "embed_entity_rich"
    EMBED_ABOVE_THRESHOLD = "embed_above_threshold"
...
def remove_duplicate_table_text(input_text: str, table_data: Any) -> str:
    ...
    start_idx = input_text.find(first_term)
    if start_idx == -1:
        return input_text
    ...
    for term in search_terms[1:15]:  # check more terms for higher confidence
        pos = input_text.find(term, current_pos)
    ...
    if matches >= 3:
        segment = input_text[start_idx:last_match_pos]
        ...
        if last_cells:
            for lc in reversed(last_cells):
                ...
                lpos = input_text.find(str(lc), last_match_pos)
                if lpos != -1 and (lpos - last_match_pos) < 2000:
                    final_end_pos = lpos + len(str(lc))
                    break
 
        processed_text = input_text[:start_idx].rstrip() + "\n\n" + input_text[final_end_pos:].lstrip()
...
# ---------------------------
def get_or_create_document(conn, external_id, title, version, metadata=None):
    with conn.cursor() as cur:
        ...
        cur.execute("""
            INSERT INTO rag.documents (id, external_id, title, version, metadata)
            VALUES (%s, %s, %s, %s, %s)
            RETURNING id
        """, (doc_id, external_id, title, version, Json(metadata) if metadata else None))
        conn.commit()
                ...
                for entity_name, section_map in entity_map_by_level.items():
                    ...
                    for scope_id, entity_value in section_map.items():
                        if entity_name and entity_value:
                            # Create a proper path key that can be matched
                            # The scope_id should match the section IDs in the hierarchy
                            path_key = (1, norm(scope_id))  # Use scope_id for matching
...
# --------------------------
def process_pdf_input(input_source: Optional[str]) -> str:
    """
...
# INGEST MAIN
# --------------------------
def ingest_optimized(pdf_input: Optional[str] = None, doc_id: Optional[str] = None, doc_title: Optional[str] = None, 
                     doc_version: Optional[str] = None, input_file: Optional[str] = None, entities_file: Optional[str] = None, 
                     keywords_file: Optional[str] = None, domain_id: Optional[str] = None):
    """
    ...
    Args:
        ...
        domain_id: Optional domain/ontology ID to associate with this document
    """
    global INPUT_FILE, ENTITIES_FILE, KEYWORDS_FILE, node_cache
    
    # Load ontology config for embedding strategy
    ontology_config = None
    try:
        from domain_aware_rag.ontology.registry import OntologyRegistry
        registry = OntologyRegistry()
        if domain_id:
            ontology_config = registry.get_ontology(domain_id)
        else:
            ontology_config = registry.get_active_ontology()
    except Exception as e:
        logger.warning(f"Could not load ontology registry during ingestion: {e}")

    if ontology_config:
        effective_domain_id = ontology_config.domain_id
        strategy = ontology_config.get_effective_embedding_strategy()
        threshold = ontology_config.entity_threshold
    else:
        effective_domain_id = domain_id or "insurance"
        strategy = "embed_entity_rich"  # Default selective strategy
        threshold = 1

    try:
        strategy_enum = EmbeddingStrategy(strategy)
    except ValueError:
        logger.warning(f"Unknown embedding strategy '{strategy}'. Falling back to embed_entity_rich.")
        strategy_enum = EmbeddingStrategy.EMBED_ENTITY_RICH

    logger.info(f"[Ingestion] Using domain: {effective_domain_id} (strategy: {strategy_enum.value}, threshold: {threshold})")
    ...
    try:
        ...
        doc_metadata = {"domain_id": effective_domain_id}
        ...
        def traverse(level_data, current_level, parent_id, path_titles):
            ...
            for title, node_data in level_data.items():
                ...
                # Try to match by scope_id if available
                path_key = (1, norm(title))  # Use title for matching
                entities = entity_map.get(path_key, [])
                
                # If no entities found, try matching with the full path
                if not entities:
                ...
                should_embed = FORCE_EMBED_ALL or strategy_enum == EmbeddingStrategy.EMBED_ALL
                if strategy_enum == EmbeddingStrategy.EMBED_ENTITY_RICH:
                    should_embed = should_embed or bool(entities)
                elif strategy_enum == EmbeddingStrategy.EMBED_ABOVE_THRESHOLD:
                    should_embed = should_embed or len(entities) >= threshold
                ...
                if should_embed:
                    ...
                    if LOG_DEBUG_NO_ENTITY_MISSES:
                        logger.debug(f"No embedding generated for hierarchy='{hierarchy_string}' (strategy={strategy_enum.value})")
